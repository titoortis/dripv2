...
// The Seedance image-to-video task body is a single shape. The provider
// reads the image's role ("first_frame" vs "reference_image") off the
// `content[*].role` discriminator; we always send exactly one image URL,
// which is the user's uploaded source image. The two modes share the
// same body otherwise.
...
export type CreateImageToVideoInput = {
  ...
  imageUrl: string;
  /** Provider-side content slot label for the image. */
  role: ImageRole;
  ratio: string;
  resolution: string;
  durationSec: number;
...
export const seedance: SeedanceClient = {
  async createImageToVideoTask(input) {
    // Single image entry. `role` is the only field that differs between
    // the first_frame and reference_image modes — the URL is the same
    // user-uploaded source image either way.
